...
const TransferUploadDialog = ({ open, setOpen }) => {
  ...
  const cleanUpPublishErrorMessage = (error) => {
    ...
    cleaned = cleaned.replace(/^Upload /, 'Project ')
    return cleaned
  }
  useEffect(() => {
          ...
          <DialogContentText style={{ marginTop: 15, marginBottom: 5 }}>Access Token* <span>
            <Tooltip title="Access Token help">
              <HelpButton
                ...
                text={`
The access token is a temporary credential used to securely authorize the publish action to an external OASIS${oasis ? ' or the central NOMAD' : ''}. It is generated using the authenticated user's credentials, ensuring that the project is transferred on behalf of that specific user. Check the [documentation](https://nomad-lab.eu/prod/v1/docs/howto/manage/program/api.html#authentication) to know how to get an access token from the transfer target.
`}
              />
            </Tooltip>
