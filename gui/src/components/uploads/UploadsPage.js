...
export const help = `
NOMAD allows you to create projects for data. After project creation, NOMAD will process your data: it will
...
#### Prepare and add files

While the project is in the staging area, you and the co-authors can add and remove files
to it. You can add plain files, or \`*.zip\` or \`*.tar.gz\` archives. We encourage you
...
your hard drive. Alternatively, you can add files via the given shell command.
...
Please add multiple archives, if you have more than 32 GB of data to add.
    ...
    <div className={classes.commandContainer} role='upload-commands'>
      ...
      <Tooltip title="Alternative shell commands">
        <HelpButton maxWidth="md" heading="Alternative shell commands" content={`
          ...
          \`\`\`
          To \`tar\` and add multiple folders in one command, use (Unix systems only):
          \`\`\`
...
export function UploadsPage() {
  ...
  // A map to store whether each of the project object's main author is the current user
  ...
  // boolean to disable the Delete button if any of the project's main author is not the current user
  ...
  return (
    <Page loading={!(data && uploadCommands)}>
      <Box marginBottom={2}>
        <Typography>
          You can either create a project and add files through the browser:
        </Typography>
      </Box>
      <Box alignItems='center' style={{display: 'flex'}}>
        ...
        <Box display="inline-block" marginLeft={2}>
          {isDisabled && <Typography color="error" role='error-maximum-number-of-unpublished'>
            You have reached maximum number of unpublished projects!
          </Typography>}
        </Box>
      </Box>
      <Box marginTop={4}>
      ...
      <Box>
        <Typography>
          Visit our documentation on <UploadDocumentation>adding files</UploadDocumentation> or show <SupportedCodes>supported codes</SupportedCodes>.
        </Typography>
      </Box>
